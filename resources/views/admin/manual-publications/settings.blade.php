            ...
            @foreach($accounts as $account)
                <form method="POST" action="{{ route('admin.manual-publications.settings.accounts.update', ['accountId' => $account->id]) }}" class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                    ...
                    <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                        ...
                        <input type="url" name="editor_url" value="{{ $account->editor_url }}" maxlength="1000" class="sm:col-span-2 rounded-md border-gray-300 text-sm shadow-sm" placeholder="平台编辑入口">
                        <input name="account_uid" value="{{ $account->account_uid }}" maxlength="255" class="rounded-md border-gray-300 text-sm shadow-sm" placeholder="账号 UID">
                        <input name="homepage_identifier" value="{{ $account->homepage_identifier }}" maxlength="255" class="rounded-md border-gray-300 text-sm shadow-sm" placeholder="主页标识">
                        <label class="flex items-center gap-2 rounded-md border border-gray-200 px-3 py-2 text-sm text-gray-700"><input type="hidden" name="browser_adapter_enabled" value="0"><input type="checkbox" name="browser_adapter_enabled" value="1" @checked($account->browser_adapter_enabled) class="rounded border-gray-300 text-blue-600">启用浏览器适配器（只填不发）</label>
