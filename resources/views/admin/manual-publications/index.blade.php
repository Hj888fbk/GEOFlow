                ...
                <section>
                    ...
                    <div id="launch-accounts" class="mt-2 max-h-80 space-y-2 overflow-y-auto pr-1">
                        @foreach($accounts as $account)
                            @php $ready = $account->browser_adapter_enabled && filled($account->editor_url) && (filled($account->profile_url) || filled($account->account_uid) || filled($account->homepage_identifier)); @endphp
                            <label data-persona="{{ $account->persona_id }}" data-ready="{{ $ready ? '1' : '0' }}" class="launch-account flex items-center gap-3 rounded-lg border border-gray-200 px-3 py-2 {{ $ready ? '' : 'opacity-60' }}">
                                ...
                                <span class="text-xs font-semibold {{ $ready ? 'text-emerald-700' : 'text-amber-700' }}">{{ $ready ? '就绪' : '待绑定' }}</span>
                            </label>
                        @endforeach
                    </div>
                </section>
        ...
        <div class="mt-6 space-y-3">
            @foreach($selectedBatch->publications as $publication)
                <details class="rounded-lg border border-gray-200 p-4" open>
                    <summary class="cursor-pointer font-semibold text-gray-900">{{ __('admin.manual_publications.platform.'.$publication->platform) }} · {{ $publication->account?->account_name ?? '未绑定账号' }}</summary>
