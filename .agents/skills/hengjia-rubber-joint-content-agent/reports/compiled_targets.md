...
| --- | --- | --- | --- | --- | --- | --- |
| `openai` | `pass` | OpenAI-style interface metadata plus neutral Agent Skills source | `metadata-adapter` | `none` | `candidate JSON plus deterministic local validation; no publish adapter` | targets/openai/adapter.json, targets/openai/agents/openai.yaml |
| `generic` | `pass` | Agent Skills compatible neutral package | `agent-skills-compatible` | `none` | `neutral source contract plus deterministic local validation; no publish adapter` | targets/generic/adapter.json |
| `agent-skills-compatible` | `pass` | Agent Skills standard source tree | `neutral-agent-skills-source` | `none` | `neutral source contract plus deterministic local validation; no publish adapter` | SKILL.md, agents/interface.yaml |
